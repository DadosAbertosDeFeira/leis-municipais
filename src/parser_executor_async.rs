...
use crate::parser_async::parse_html_to_lei;
...
pub async fn parse_on_directory_async(directory_path: &str) -> (HashMap<String, Folder>, Vec<Lei>) {
    ...
    for entry in walker.filter_entry(|entry| is_not_hidden(entry)).skip(1) {
        ...
        if entry.file_type().is_dir() {
            ...
            let lei = unwrap_or_print_err!(
                parse_html_to_lei(file_path, current_folder.to_string()).await
            );
